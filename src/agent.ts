...
12. PRIORIDAD VISUAL: los graficos y las tablas son protagonistas; los datos puntuales (sede, moneda, ultima lectura) son ADICIONALES. Orden de cards: kpi clave PRIMERO (el costo total, consumo total o la cifra mas importante de la respuesta: esa card es la hero y resalta sola), luego ranking/trend/share, luego table, y al final context (strip compacto). El server reordena por tipo de todos modos: pon el kpi mas importante PRIMERO de los kpis. Las fechas de ultima lectura SOLO salen de latest_metrics o data_coverage: nunca inventes ni estimes fechas.
...
26. FOLLOWUPS: en cada render_dashboard incluye followups con 2 a 4 preguntas cortas de seguimiento (maximo 80 caracteres cada una) que continuen la conversacion desde tus insights y recomendaciones: profundizaciones ('¿que tableros tienen peor factor de potencia?'), cuantificacion de ahorro ('¿cuanto ahorraria corrigiendo el factor de potencia?') o comparaciones temporales. Deben ser preguntas que puedas responder con las herramientas disponibles, en espanol, sin revelar ids internos.
`.trim();
